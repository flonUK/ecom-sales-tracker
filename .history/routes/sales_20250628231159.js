...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    if (days_back && days_back !== 'all') {
      ...
      // Format the cutoff date as YYYY-MM-DD for consistent comparison
      const cutoffDateStr = cutoffDate.toISOString().split('T')[0];
      whereClause += ` AND (date(sale_date) >= ? OR sale_date >= ?)`;
      params.push(cutoffDateStr, cutoffDate.toISOString());
    }
...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    // Add date filter - fix to handle ISO timestamps properly
    if (days_back && days_back !== 'all') {
      const days = parseInt(days_back.replace('d', ''));
      const cutoffDate = new Date();
      cutoffDate.setDate(cutoffDate.getDate() - days);
