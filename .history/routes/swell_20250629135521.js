  ...
  try {
    ...
    db.get(query, [req.user.userId, 'swell'], async (err, credentials) => {
      ...
      try {
        ...
        // Handle "all" for lifetime filtering
        if (days_back === 'all' || days_back === 'all time') {
          // Set start date to a very old date to get all orders
          startDate.setFullYear(2020); // Start from 2020 to get all orders
        } else {
    ...
    db.get(query, [req.user.userId, 'swell'], async (err, credentials) => {
      ...
      try {
        ...
        while (hasMoreOrders) {
          ...
          console.log(`Manual sync Page ${page-1}: Found ${orders.length} orders, Total so far: ${allOrders.length}/${totalCount}`);
        }

        console.log(`Manual sync: Found ${allOrders.length} Swell orders (total available: ${totalCount})`);
