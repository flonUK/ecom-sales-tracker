    ...
    for (const connection of connections) {
      try {
        ...
        if (connection.platform === 'swell') {
          // Call Swell sync by making a request to the Swell sync endpoint
          const swellSync = require('./swell');
          ...
          const mockRes = {
            status: (code) => ({
              json: (data) => {
                syncResult = { code, data };
              }
            }),
            json: (data) => {
              syncResult = { code: 200, data };
            }
          };
          
          // Call the Swell sync endpoint directly
          await swellSync.post('/sync', mockReq, mockRes);
          
          if (syncResult && syncResult.code === 200) {
            results.push({
              platform: connection.platform,
              status: 'success',
              message: syncResult.data.message || 'Sync completed',
              salesCount: syncResult.data.totalSales || 0
            });
            totalSynced++;
          } else {
            results.push({
              platform: connection.platform,
              status: 'error',
              message: syncResult?.data?.error || 'Sync failed'
            });
          }
        } else {
