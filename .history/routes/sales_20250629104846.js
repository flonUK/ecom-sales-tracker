...
router.get('/analytics', verifyToken, async (req, res) => {
  try {
    ...
    const newCustomers = customerData.filter(c => {
      const firstOrder = new Date(c.first_order);
      const cutoffDate = new Date();
      cutoffDate.setDate(cutoffDate.getDate() - parseInt(days_back.replace('d', '')));
      return firstOrder >= cutoffDate;
    }).length;
    
    const returningCustomers = customerData.filter(c => c.order_count > 1).length;
...
router.get('/customers', verifyToken, async (req, res) => {
  try {
    ...
    const frequentBuyers = customers.filter(c => c.order_count >= 3).length;
    const highValueCustomers = customers.filter(c => c.total_spent >= 100).length;
    ...
    res.json({
      customers: customers.map(customer => ({
        ...
        firstOrder: customer.first_order,
        lastOrder: customer.last_order,
        customerType: customer.order_count >= 3 ? 'Frequent Buyer' : 
                     customer.total_spent >= 100 ? 'High Value' : 'Regular'
      })),
      summary: {
        totalCustomers,
        totalRevenue,
