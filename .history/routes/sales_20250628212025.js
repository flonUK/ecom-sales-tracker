    ...
    for (const connection of connections) {
      try {
        ...
        if (connection.platform === 'swell') {
          // Call Swell sync by making a request to the local Swell sync endpoint
          const response = await fetch(`http://localhost:5001/api/swell/sync`, {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            results.push({
              ...
              salesCount: syncResult.totalSales || 0
            });
            totalSynced++;
          } else {
            results.push({
