...
router.post('/sync', verifyToken, async (req, res) => {
  try {
    ...
    const query = `
      SELECT platform, store_id, public_key, secret_key 
      FROM api_credentials 
      WHERE user_id = ? AND is_active = 1
    `;
    
    const credentials = await dbHelper.all(query, [req.user.userId]);
    
    if (credentials.length === 0) {
      return res.status(404).json({ error: 'No platforms connected' });
    }
    ...
    for (const cred of credentials) {
      try {
        if (cred.platform === 'swell') {
          // Sync Swell data
          const { swell } = require('swell-node');
          swell.init(cred.store_id, cred.secret_key);
          ...
          for (const order of orders) {
            if (order.items && Array.isArray(order.items)) {
              for (const item of order.items) {
                ...
                await dbHelper.run(insertQuery, [
                  req.user.userId,
                  'swell',
        ...
        console.error(`Error syncing ${cred.platform}:`, platformError);
        results.push({
