  ...
  try {
    ...
    db.get(query, [req.user.userId, 'swell'], async (err, credentials) => {
      ...
      try {
        ...
        console.log(`Found ${allOrders.length} Swell orders (total available: ${ordersResponse?.count || allOrders.length})`);
  ...
  try {
    ...
    db.get(query, [req.user.userId, 'swell'], async (err, credentials) => {
      ...
      try {
        ...
        console.log(`Stats: Found ${allOrders.length} Swell orders (total available: ${ordersResponse?.count || allOrders.length})`);
