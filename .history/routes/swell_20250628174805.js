...
router.post('/connect', verifyToken, async (req, res) => {
  try {
    ...
    // Swell uses API keys in headers, not Basic auth
    ...
    if (!testResponse.ok) {
      ...
      const altResponse = await fetch(`${SWELL_API_URL}/stores/${storeId}/products`, {
        headers: {
          'X-Swell-API-Key': publicKey,
          'X-Swell-Secret-Key': secretKey,
          'Content-Type': 'application/json'
        }
      });

      if (!altResponse.ok) {
        // Try with just the public key as Bearer token
        const bearerResponse = await fetch(`${SWELL_API_URL}/stores/${storeId}/products`, {
          headers: {
        ...
        if (!bearerResponse.ok) {
          ...
          return res.status(400).json({ 
            error: 'Invalid Swell credentials. Please check your Store ID, Public Key, and Secret Key.',
            details: 'API test failed with all authentication methods'
          });
        }
      }
    }
