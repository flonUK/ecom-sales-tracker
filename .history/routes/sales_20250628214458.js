...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    const sales = await db.all(salesQuery, [...params, limit, offset]);
...
router.get('/stats', verifyToken, async (req, res) => {
  try {
    ...
    const platformData = await db.all(platformQuery, params);
    ...
    for (const connection of connections) {
      try {
        ...
        if (connection.platform === 'swell') {
          ...
          const swellResult = await swellSync.syncSwellData(userId, connection);
