  ...
  try {
    ...
    db.get(query, [req.user.userId, 'swell'], async (err, credentials) => {
      ...
      try {
        ...
        const swell = require('swell-node').init(credentials.store_id, credentials.secret_key);
          ...
          if (order.items && Array.isArray(order.items)) {
            for (const item of order.items) {
              ...
              const insertQuery = `
                ...
                (user_id, platform, order_id, item_title, price, quantity, sale_date, status, sku) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
              `;
