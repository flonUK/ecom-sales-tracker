...
router.get('/auth-url', verifyToken, (req, res) => {
  if (!ETSY_CLIENT_ID) {
    return res.status(400).json({ 
      error: 'Etsy OAuth not configured. Please set ETSY_CLIENT_ID in environment variables.',
      demoMode: true
    });
  }
...
// Connect Etsy shop using shop URL (fallback for demo mode)
router.post('/connect', verifyToken, async (req, res) => {
  try {
    ...
    // Demo mode - create mock shop data
    const mockShop = {
      shop_id: shopName,
...
router.get('/sales', verifyToken, async (req, res) => {
  try {
    ...
    for (const shop of shops) {
      try {
        // If shop has OAuth token, get real sales data
    ...
    res.json({ 
      ...
      message: allSales.some(sale => sale.order_id.startsWith('demo-')) ? 
        'Demo mode: Some shops showing mock data. Connect with OAuth for real sales data.' :
        'Real sales data from Etsy OAuth'
    });
