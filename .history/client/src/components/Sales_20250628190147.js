...
const Sales = () => {
  ...
  const syncData = async () => {
    try {
      ...
      const platforms = ['swell', 'etsy', 'ebay', 'amazon'];
      const syncPromises = platforms.map(platform => 
        fetch(`/api/${platform}/sales`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }).catch(err => {
          console.log(`Failed to sync ${platform}:`, err);
          return null;
        })
      );
      ...
      <div className="sales-table-container">
        ...
        {Array.isArray(sales) && sales.length > 0 ? (
          ...
          <div className="empty-state">
            ...
            <p>Your platforms are connected but no sales data has been synced yet. Click "Sync Data" to fetch your latest sales from connected platforms.</p>
