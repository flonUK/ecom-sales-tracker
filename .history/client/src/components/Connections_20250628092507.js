...
const Connections = () => {
  ...
  const handleConnect = async (platform) => {
    ...
    try {
      ...
      if (response.data.demoMode) {
        ...
        const demoResponse = await axios.post(`/api/${platform}/connect`, {});
        setIsConnecting(null);
        queryClient.invalidateQueries(['connections']);
        toast.success(`${platform} connected successfully in demo mode!`);
        return;
      }
      ...
      setIsConnecting(null);
      if (error.response?.data?.demoMode) {
        // Try demo mode connection
        try {
          const demoResponse = await axios.post(`/api/${platform}/connect`, {});
          queryClient.invalidateQueries(['connections']);
          toast.success(`${platform} connected successfully in demo mode!`);
        } catch (demoError) {
          toast.error(`Failed to connect ${platform}: ${demoError.response?.data?.error || 'Unknown error'}`);
        }
      } else {
  ...
  const handleConnectEtsy = async () => {
    ...
    try {
      const response = await axios.post('/api/etsy/connect', { shopUrl: etsyShopUrl.trim() });
      
      if (response.data.requiresOAuth) {
        // OAuth is required - start OAuth flow
        const authResponse = await axios.get('/api/etsy/auth-url');
        window.open(authResponse.data.authUrl, '_blank', 'width=600,height=600');
        
        // Poll for connection status
        const checkConnection = setInterval(async () => {
          try {
            const shopsResponse = await axios.get('/api/etsy/shops');
            if (shopsResponse.data.shops.length > 0) {
              clearInterval(checkConnection);
              setShowEtsyModal(false);
              setEtsyShopUrl('');
              queryClient.invalidateQueries(['connections']);
              toast.success('Etsy connected successfully with OAuth!');
            }
          } catch (error) {
            // Continue polling
          }
      ...
      toast.error(`Failed to connect Etsy: ${error.response?.data?.error || 'Unknown error'}`);
    }
  };
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
              <p className="text-sm text-blue-800 mb-2">
                <strong>Two Connection Options:</strong>
              </p>
              <ul className="text-xs text-blue-700 space-y-1">
                <li>• <strong>OAuth (Recommended):</strong> Real sales data, requires Etsy app setup</li>
                <li>• <strong>Demo Mode:</strong> Mock data for testing, no setup required</li>
              </ul>
            </div>
